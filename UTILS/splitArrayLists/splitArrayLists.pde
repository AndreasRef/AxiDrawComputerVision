...
void setup() {
  ...
  //println(vectors.indexOf(zeroVector));
  ...
  for (int i = 0; i<breakPoints.size() + 1; i++) { //2 create breakPoints.size()+1 new ArrayLists 
    ...
    if (i == 0) { //first sublist
     mySubList = new ArrayList(vectors.subList(0, breakPoints.get(0)));
...
/* Works for single breakpoint
  //2 create breakPoints.size()+1 new arrayLists 
  println(breakPoints.size());
  int lastBreakpoint = 0;
  for (int i = 0; i<breakPoints.size() + 1; i++) {
    ArrayList<PVector> mySubList = new ArrayList();
    if (i<breakPoints.size()) { //first sublist
     mySubList = new ArrayList(vectors.subList(0, breakPoints.get(0)));
     lastBreakpoint = breakPoints.get(i);
    } else { //last sublist
     mySubList = new ArrayList(vectors.subList(lastBreakpoint + 1 , vectors.size()));
...
*/
  ...
  ArrayList<PVector> myHead = new ArrayList(vectors.subList(0, vectors.indexOf(zeroVector)));
  ArrayList<PVector> myTails = new ArrayList(vectors.subList(0, vectors.indexOf(zeroVector)));
  //println(myHead);
  //println(myTails);
}
