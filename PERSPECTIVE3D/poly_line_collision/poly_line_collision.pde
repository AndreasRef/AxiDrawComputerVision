//https://discourse.processing.org/t/check-if-line-intersects-a-polygon/16978/8
...
void draw() {
  ...
  boolean hit = contains(vertices, x1, y1);
  //if (hit) hit = polyLine(vertices, x1, y1, x2, y2); 
  if (hit) fill(255, 150, 0);
  else fill(0, 150, 255);
  
  text("hit " + str(hit), 10, 10);
...
boolean contains(PVector[] vertices, float x, float y){
    int N = vertices.length;
    boolean isInside = false;
    for (int i = 0; i < N; i++){
        PVector v1 = vertices[(i+1)%N];
